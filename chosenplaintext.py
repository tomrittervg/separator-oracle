...
def getExceptionType(ciphertext, verbosity=0):
    ...
    oracle_server.request("GET", '/plaintext/decrypt/' + ciphertext)
...
def decrypt(valid_ciphertext, seperator_vector, verbosity=0):    
    ...
    ciphertext_onelessseperator = valid_ciphertext[:] #Without the [:] it's copy by reference
...
def reencrypt(valid_ciphertext, decrypted_block, newPlaintext, seperator_vector, sep, verbosity=0):    
    if len(newPlaintext) != len(decrypted_block):
        raise Exception("Cannot transform ciphertext, lengths don't match.")
    newciphertext = bytearray()
    for i in range(len(decrypted_block)):
        if seperator_vector[i] == 1:
            newciphertext.append(valid_ciphertext[i] ^ ord(sep) ^ ord(newPlaintext[i]))
        else:
            newciphertext.append(valid_ciphertext[i] ^ decrypted_block[i] ^ ord(newPlaintext[i]))
            
    answer, response = getExceptionType(newciphertext, verbosity)
    if verbosity > 1: print "[2]", printExceptionType(answer)
    if verbosity > 1: print "\t[2]", "Server Response:", response
    
    return b64encode(arrayToStr(newciphertext))
            
#Tom|User
valid_ciphertext = bytearray(b64decode("ybSGHN6zW6c="))
#Jack|Ano
#valid_ciphertext = bytearray(b64decode("17qIC/eBULo="))
    
seperator_vector = findSeperators(valid_ciphertext, 0)
decrypted_block = decrypt(valid_ciphertext, seperator_vector, 0)

print "What would you like it to decrypt to?"
newPlaintext = "To|Admin"
print "Making it decrypt to", newPlaintext

newciphertext = reencrypt(valid_ciphertext, decrypted_block, newPlaintext, seperator_vector, "|", 0)

print "The correct ciphertext is (base64-ed)", newciphertext
print "http://YOURWEBSERVER.COM:9999/plaintext/decrypt/" + newciphertext.replace('/', '%2F').replace('+', '%2B')